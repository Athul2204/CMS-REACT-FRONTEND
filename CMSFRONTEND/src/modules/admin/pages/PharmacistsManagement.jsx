...
import { getPharmacistList, patchStaff } from "../api/adminApi";

const ROLE_CONFIG = {
  role: "Pharmacist",
  endpoint: "/api/administration/pharmacist/",
  idKey: "id",
  extraFields: [
    { name: "license_number", label: "License Number *", type: "text", placeholder: "e.g. LIC-12345", default: "" },
  ],
  accentColor: "#fbbf24",
};
...
const PharmacistsManagement = () => {
  ...
  const [currentUrl, setCurrentUrl] = useState("/api/administration/pharmacist/");
  ...
  const fetchData = async (url) => {
    ...
    setError("");
    try {
      const res = await getPharmacistList(url);
      setData(res.results || []);
      setNext(res.next);
  ...
  const filtered = data.filter((r) => {
    ...
    return name.includes(search.toLowerCase()) || (r.license_number || "").toLowerCase().includes(search.toLowerCase());
  });

  const withActive = filtered.map((row) => ({ ...row, is_active: row.staff?.is_active ?? true }));

  return (
    <AdminLayout title="Pharmacists">
      ...
      <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
        <input type="text" placeholder="Search by name or license…" value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="bg-[#0d1629] border border-[#1e2d4a] rounded-lg px-4 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#fbbf24] w-72 transition" />
        <button onClick={() => { setSelected(null); setModalOpen(true); }}
          className="flex items-center gap-2 px-4 py-2 bg-[#fbbf24] text-[#060d1a] text-sm font-semibold rounded-lg hover:bg-[#fcd34d] transition">
      ...
      <DataTable columns={COLUMNS} data={withActive} loading={loading}
        ...
        onToggleActive={handleToggleActive} idKey="id" />
      ...
      {modalOpen && (
        <RoleModal record={selected} onClose={() => setModalOpen(false)}
          onSaved={() => { setModalOpen(false); fetchData("/api/administration/pharmacist/"); }}
